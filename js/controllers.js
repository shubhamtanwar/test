...
.controller('SendFeedbackCtrl', ['$scope', '$rootScope', '$http', '$location',
    ...
    .controller('LaunchCtrl', ['$rootScope', '$scope', '$http', '$location', '$window', '$sce', '$route', '$compile','$routeParams',
        function ($rootScope, $scope, $http, $location, $window, $sce, $route, $compile, $routeParams) {
            ...
            $scope.launchApp = function() {
                ...
                var networkState = navigator.connection.type;
                //alert(navigator.connection.type+" -- LaunchCtrl");
                if (networkState == "none") {
                  $.unblockUI();
                    $location.path("/noNetwork/"+window.location.href.split('#')[1].split('/')[2]);
                    return;
    ...
    .controller('noNetworkCtrl', ['$rootScope', '$scope', '$http', '$location', '$window', '$sce', '$route', '$compile','$routeParams',
        function ($rootScope, $scope, $http, $location, $window, $sce, $route, $compile, $routeParams) {
            $rootScope.noNetworkPage = true;
            $scope.checkNetwork = function(){
              $("#tryAgainBtn").html($.blockUI.defaults.message);
              try{
              if (navigator.connection.type == "none") {
                  ...
                  /*navigator.notification.alert("Oops! You are not connected to Internet. Please check your settings and try again",
                      null, 'No Network', 'Ok');*/
                  return;
              }else if($routeParams.pageid == "reloadApp" || $routeParams.pageid === "undefined"){
                $location.path('/home');
                $route.reload();
              }else{
                console.log("pageid: "+$routeParams.pageid);
                $location.path('/app/'+$routeParams.pageid+"/"+$routeParams.pageid);
                $route.reload();
              }
            }catch(e){alert(e);}
            }
